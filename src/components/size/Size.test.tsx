...
it("Control Size call event handler", () => {
  const probe = jest.fn();
  render(<Size minSize={10} maxSize={50} onSizeChange={probe} defSize={50} />);
  const control = screen.getByTestId("size");
...
it("Control Size check min boundary", () => {
  ...
  render(
    <Size minSize={minSize} maxSize={50} onSizeChange={probe} defSize={50} />
  );
  const control = screen.getByTestId("size");
...
it("Control Size check max boundary", () => {
  ...
  render(
    <Size minSize={10} maxSize={maxSize} onSizeChange={probe} defSize={50} />
  );
  const control = screen.getByTestId("size");
